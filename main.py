...
conn = mysql.connector.connect(host="127.0.0.1",user="root",password="",database="teach-me")
cursor = conn.cursor()
...
def sign_for_course(user_id, id_course, result, progress, difficulty, requirement, img):
    cursor = conn.cursor()
    ...
    if isinstance(requirement, bytes):
        requirement = requirement.decode('utf-8')

    sql = """
        INSERT INTO courses_taken 
        (user_id, course_id, result, progress, difficulty, requirement, img)
        VALUES (%s, %s, %s, %s, %s, %s, %s)
    """
    values = (user_id, id_course, result, progress, difficulty, requirement, img)
...
def get_first_chapitre(id_course):
    ...
    sign_for_course(user_id,id_course,result.encode('utf-8'),0,get_difficulty(id_course),get_requiremment(id_course),img)
...
def quote(path):
        return f'"{path}"' if platform.system() == 'Windows' else path
def backup():
    DB_NAME = 'teach-me'
    BACKUPPATH = 'C:/Users/prajw/Desktop/db_backups/'
    DB_HOST = 'localhost' 
    DB_USER = 'phpmyadmin'
    ...
    dumpcmd = f"mysqldump -h {DB_HOST} -u {DB_USER} -p{DB_USER_PASSWORD} {db} > {quote(TODAYBACKUPPATH)}/{db}.sql"
    gzipcmd = f"gzip {quote(TODAYBACKUPPATH)}/{db}.sql"
...
def forum_question():
    ...
    x=list()
    x = list(x)  # assuming x is a map object
...
def quizz(id):
    ...
    if result:
        ...
        if request.method=="POST":
            ...
            result_list=list()
            result_list = list(zip(correct_answers,chosen_answers))
...
def Felicitation(id):
    ...
    mark = cursor.fetchall()
    if not mark:
        mark = [[0]]  # or [[None]] depending on what makes sense
